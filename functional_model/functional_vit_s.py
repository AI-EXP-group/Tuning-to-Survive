...
def LSA(weights, x, dim, idx, heads=8, dim_head=64, dropout=0.0):
    # scale = dim_head**-0.5
    ...
    temperature = weights[f"transformer.layers.{idx}.0.fn.temperature"]
    dots = torch.matmul(q, k.transpose(-1, -2)) * temperature.exp()
...
def transformer(
    ...
    for i in range(depth):
        ...
        out1 = LSA(weights, out1, dim, i, heads, dim_head, dropout)
...
def functional_vit_s(
    ...
    x = F.layer_norm(
        x,
        weight=weights["to_patch_embedding.to_patch_tokens.1.weight"],
        bias=weights["to_patch_embedding.to_patch_tokens.1.bias"],
        normalized_shape=(patch_size * patch_size * 5 * channels,),
    )
    x = F.linear(
        x,
        weight=weights["to_patch_embedding.to_patch_tokens.2.weight"],
        bias=weights["to_patch_embedding.to_patch_tokens.2.bias"],
    )
    b, n, _ = x.shape
