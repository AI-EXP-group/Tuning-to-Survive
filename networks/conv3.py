...
#
#     def __init__(
#         self, x_dim: int = 28, n_channels: int = 1, n_classes: int = 10, mult: int = 2,
#     ):
...
class Conv3(nn.Module):
    ...
    def __init__(
        ...
        self.line = nn.Linear(256 * mult, self.n_classes)

    def forward(self, x, return_rep=False):
        if self.downsample:
            x = F.interpolate(x, 32)
        feature = self.net(x)
        out = self.line(feature)
        if return_rep==False:
            return out
        else:
            return out,feature
